...
export interface ejerciciosRules {
  Id: number;
  nombre: string;
  descripcion: string;
  etiquetas: string[];
...
export class EjerciciosService {
  ...
  obtenerEjercicio(id:number):ejerciciosRules{
	...
	buscarEjercicio(termino:string){
		...
		for(let ejercicio of this.ejercicios){
			let nombre = ejercicio.nombre.toLowerCase();
			if(nombre.indexOf(termino) >= 0){
				ejerArr.push(ejercicio)
			}
